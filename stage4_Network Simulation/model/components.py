...
class Bridge(Infra):
    ...
    def get_delay_time(self):
        if self.delay:
            # depending on the bridge length, return a specific value for each truck arriving at the broken bridge
            if self.length < 10:
...
class Vehicle(Agent):
    ...
    def arrive_at_next(self, next_infra, location_offset):
        ...
        self.location_offset = location_offset
        self.location.vehicle_count += 1
        if isinstance(next_infra, Link):
            next_infra.trucks_passed += 1
